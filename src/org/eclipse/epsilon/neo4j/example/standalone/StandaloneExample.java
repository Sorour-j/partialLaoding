...
public class StandaloneExample implements AutoCloseable {
	...
	public static void main(String... args) throws Exception {
		...
		// TestDBMS
		try (StandaloneExample greeter = new StandaloneExample("neo4j+s://demo.neo4jlabs.com:7687", "twitter", "twitter")) {
			greeter.load();
		}
	}
	...
	public void load() throws Exception {
		...
		//	for (int i =0; i<10; i++) {
			...
			String q = "MATCH p=(s:User)-[:FOLLOWS*]->(t:User)"+
			"WHERE NOT (t)-[:FOLLOWS*]->()" +
			"RETURN nodes(p), [n in nodes(p)|n.name] as names limit 10";

			Result re3 = session.run(q);
			...
			while (re3.hasNext()) {
				...
				while (i < record.get(0).size()) {
					System.out.println(record.get("names").get(i));
					i++;
				}
//				System.out.println(record.get(0).get(1));
				//record.get("name");
				//record.get("scname");
			}
		}
